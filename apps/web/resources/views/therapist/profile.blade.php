...
{{-- ===================== Tabs ===================== --}}
<nav class="sticky top-16 z-30 border-b border-ink-100 bg-white/95 shadow-sm backdrop-blur" aria-label="{{ $therapist['name'] }}">
        ...
        <div class="min-w-0 space-y-9">
            ...
            <section id="overview" aria-labelledby="overview-heading" class="scroll-mt-32">
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm">
                    <p class="leading-relaxed text-ink-700">{{ $therapist['about'] }}</p>
                    @if ($therapist['experience_note'])
                        <p class="mt-4 border-t border-ink-100 pt-4 text-sm text-ink-500">{{ $therapist['experience_note'] }}</p>
                    @endif
                </div>
            </section>
            ...
            <section aria-labelledby="specialties-heading" class="scroll-mt-32">
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm">
                    <dl class="space-y-5">
                        @foreach ([
                            ['label' => __('therapist.specialty_focus'), 'items' => $therapist['specialties'], 'tone' => 'bg-leaf-50 text-leaf-700'],
                            ['label' => __('therapist.massage_types'), 'items' => $therapist['massage_types'], 'tone' => 'bg-ink-50 text-ink-700'],
                            ['label' => __('therapist.client_focus'), 'items' => $therapist['client_focus'], 'tone' => 'bg-ember-50 text-ember-700'],
                        ] as $group)
                            <div>
                                <dt class="text-sm font-bold text-ink-900">{{ $group['label'] }}</dt>
                    ...
                    <p class="mt-5 border-t border-ink-100 pt-4 text-xs text-ink-400">{{ __('therapist.capability_note') }}</p>
                </div>
            </section>
                        ...
                        @foreach ($therapist['services'] as $service)
                            <li class="flex items-center gap-4 px-4 py-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-sm text-ink-400">{{ __('therapist.minutes', ['count' => $service['duration']]) }} · {{ $service['context'] }}</p>
                                </div>
                                <p class="shrink-0 text-lg font-black text-ember-600">{{ $service['price'] }}</p>
                            </li>
                        @endforeach
                    </ul>
                    <p class="mt-3 text-xs text-ink-400">{{ __('therapist.last_confirmed', ['date' => $therapist['last_confirmed']]) }}</p>
                @else
                    <div class="rounded-2xl border border-dashed border-ink-200 bg-ink-50/50 p-6 text-center">
                        <h3 class="font-bold text-ink-700">{{ __('therapist.no_services_title') }}</h3>
                        <p class="mx-auto mt-1.5 max-w-md text-sm text-ink-500">{{ __('therapist.no_services_text') }}</p>
                    </div>
                @endif
            </section>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    @foreach ($therapist['affiliations'] as $context)
                        <article class="rounded-2xl border border-ink-100 bg-white p-4 shadow-sm">
                            <h3 class="font-bold text-ink-950">{{ $context['is_independent'] ? __('therapist.independent_practice') : $context['name'] }}</h3>
                            ...
                            <p class="mt-2.5 text-sm text-ink-500">{{ $context['detail'] }}</p>
                        </article>
                    @endforeach
                </div>
            </section>
                ...
                <ul class="divide-y divide-ink-100 overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm">
                    @foreach ($therapist['affiliations'] as $affiliation)
                        <li class="flex flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3.5">
                            <div class="min-w-0 flex-1">
                                <h3 class="font-bold text-ink-950">
                                    @if ($affiliation['spa_slug'])
                                        <a href="{{ route('spa.show', ['establishment_slug' => $affiliation['spa_slug']]) }}" class="transition hover:text-ember-600">{{ $affiliation['name'] }}</a>
                                    @else
                            ...
                            <span class="rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wide {{ $affiliation['confirmation'] === 'therapist_confirmed' ? 'bg-leaf-50 text-leaf-700' : 'bg-ink-50 text-ink-500' }}">
                                {{ __('therapist.confirmation_'.$affiliation['confirmation']) }}
                            </span>
                        </li>
                    @endforeach
                </ul>
                <p class="mt-3 text-xs text-ink-400">{{ __('therapist.affiliation_note') }}</p>
            </section>
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm">
                    <ul class="flex flex-wrap gap-2">
                        @foreach ($therapist['service_area'] as $area)
                            <li class="rounded-full bg-ink-50 px-3.5 py-1.5 text-sm font-semibold text-ink-700">{{ $area }}</li>
                        @endforeach
                    </ul>
                    <p class="mt-4 border-t border-ink-100 pt-4 text-xs text-ink-400">{{ __('therapist.service_area_note') }}</p>
                </div>
            </section>
                        ...
                        <ul class="space-y-2.5 text-sm text-ink-700">
                            @if ($credentials['verified_active'] > 0)
                                <li class="flex items-center gap-2.5">
                                    <svg viewBox="0 0 20 20" fill="currentColor" class="size-5 shrink-0 text-leaf-600" aria-hidden="true"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.7-9.3a1 1 0 0 0-1.4-1.4L9 10.6 7.7 9.3a1 1 0 0 0-1.4 1.4l2 2a1 1 0 0 0 1.4 0l4-4Z" clip-rule="evenodd"/></svg>
                            ...
                            @if ($credentials['submitted'] > 0)
                                <li class="flex items-center gap-2.5">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="size-5 shrink-0 text-ink-400" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l2.5 2.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                                    {{ trans_choice('therapist.credentials_submitted', $credentials['submitted'], ['count' => $credentials['submitted']]) }}
                                </li>
                            @endif
                        </ul>
                    @else
                        <p class="text-sm text-ink-500">{{ __('therapist.credentials_none') }}</p>
                    @endif
                    <p class="mt-5 border-t border-ink-100 pt-4 text-xs text-ink-400">{{ __('therapist.credentials_note') }}</p>
                </div>
            </section>
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm">
                    <div class="flex flex-col gap-8 md:flex-row">
                        <div class="shrink-0 text-center md:w-44">
                            <p class="text-sm font-bold text-ink-500">{{ __('therapist.rating_summary') }}</p>
                            @if ($therapist['rating'] !== null)
                                <p class="mt-1 text-5xl font-black text-ink-950">{{ number_format($therapist['rating'], 1) }}</p>
                                <x-rating :value="$therapist['rating']" size="size-5" class="mt-2 justify-center" />
                                <p class="mt-1.5 text-xs text-ink-400">{{ trans_choice('therapist.rating_count', $therapist['rating_count'], ['count' => $therapist['rating_count']]) }}</p>
                                <p class="text-xs text-ink-400">{{ trans_choice('therapist.review_count', $therapist['review_count'], ['count' => $therapist['review_count']]) }}</p>
                            @else
                                <p class="mt-3 rounded-xl bg-ink-50 px-3 py-4 text-sm font-semibold text-ink-500">{{ __('therapist.not_enough_ratings') }}</p>
                                <p class="mt-1.5 text-xs text-ink-400">{{ trans_choice('therapist.rating_count', $therapist['rating_count'], ['count' => $therapist['rating_count']]) }}</p>
                                <p class="mt-1.5 text-xs text-ink-400">{{ trans_choice('therapist.review_count', $therapist['review_count'], ['count' => $therapist['review_count']]) }}</p>
                            @endif
                        </div>
                        <ul class="min-w-0 flex-1 space-y-5">
                            @forelse ($therapist['reviews'] as $review)
                                <li class="flex gap-3.5 {{ $loop->last ? '' : 'border-b border-ink-100 pb-5' }}">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        @if ($review['service_received'])<p class="mt-0.5 text-xs text-ink-400">{{ $review['service_received'] }}</p>@endif
                            ...
                            @empty
                                <li class="rounded-xl bg-ink-50 p-5 text-sm text-ink-500">{{ __('therapist.no_reviews') }}</li>
                            @endforelse
                        </ul>
                    </div>
                    <div class="mt-6 flex flex-wrap justify-end gap-3">
                        ...
                        <a href="{{ route('review.therapist', ['target' => $therapist['slug']]) }}" class="inline-flex items-center gap-1 rounded-xl border border-ink-200 px-4 py-2 text-sm font-bold text-ink-700 transition hover:border-ember-300 hover:text-ember-600">
            ...
            <section id="about" aria-labelledby="verification-heading" class="scroll-mt-32">
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-ink-600">
                        @if ($therapist['is_claimed'])
                    ...
                    @unless ($therapist['is_claimed'])
                        <p class="mt-3 border-t border-ink-100 pt-3 text-sm text-ink-500">
                            {{ __('therapist.are_you_therapist', ['name' => $therapist['name']]) }}
                            <a href="{{ url('/claim/therapist') }}" class="font-bold text-ember-600 transition hover:text-ember-700">{{ __('therapist.claim_this_profile') }}</a>
                        </p>
                    @endunless
                </div>
            </section>
        ...
        <aside class="min-w-0">
            @if ($therapist['is_booking_enabled'])
                <section id="book" aria-labelledby="book-heading" class="scroll-mt-32 rounded-2xl border border-ink-100 bg-white p-4 shadow-md lg:sticky lg:top-36">
                    <h2 id="book-heading" class="text-lg font-black text-ink-950">{{ __('therapist.book_title', ['name' => $therapist['name']]) }}</h2>
                    <form class="mt-5 space-y-4" action="#book" method="get">
                        <div>
                            <label for="booking-context" class="mb-1 block text-sm font-bold text-ink-900">{{ __('therapist.select_context') }}</label>
                            <select id="booking-context" name="context" class="w-full rounded-xl border border-ink-200 bg-white px-3 py-2 text-sm text-ink-950 transition focus:border-ember-400 focus:outline-none focus:ring-2 focus:ring-ember-100">
                                @foreach ($therapist['affiliations'] as $context)
                                    @if ($context['booking_mode'])
                        ...
                        <div>
                            <label for="booking-service" class="mb-1 block text-sm font-bold text-ink-900">{{ __('therapist.select_service') }}</label>
                        ...
                        <div>
                            <label for="booking-date" class="mb-1 block text-sm font-bold text-ink-900">{{ __('therapist.select_date') }}</label>
                        ...
                        <div>
                            <label for="booking-time" class="mb-1 block text-sm font-bold text-ink-900">{{ __('therapist.select_time') }}</label>
                        ...
                        <p class="text-center text-xs text-ink-400">{{ __('therapist.book_note') }}</p>
                        <p class="rounded-xl bg-ink-50 px-3.5 py-2.5 text-center text-xs text-ink-500">{{ __('therapist.booking_mode_request') }}</p>
                    </form>
                </section>
            @else
                <section aria-labelledby="unclaimed-heading" class="rounded-2xl border border-ember-200 bg-ember-50 p-5 shadow-sm lg:sticky lg:top-36">
                    <h2 id="unclaimed-heading" class="text-lg font-black text-ink-950">{{ __('therapist.unclaimed_notice_title') }}</h2>
                    <p class="mt-2 text-sm leading-relaxed text-ink-700">{{ __('therapist.unclaimed_notice_text') }}</p>
