...
@section('content')
<h1 class="text-2xl font-black tracking-tight text-ink-950 dark:text-ink-50">{{ __('auth.password_reset_title') }}</h1>
<p class="mt-1.5 text-sm text-ink-500 dark:text-ink-400">{{ __('auth.password_reset_subtitle') }}</p>
    ...
    <div>
        ...
        <div class="relative">
            <input id="password" name="password" type="password" required autofocus autocomplete="new-password"
                   ...
                   class="w-full rounded-xl border {{ $errors->has('password') ? 'border-ember-400 dark:border-ember-500' : 'border-ink-200 dark:border-ink-700' }} bg-white px-4 py-3 pr-12 text-sm text-ink-950 transition focus:border-ember-400 focus:outline-none focus:ring-2 focus:ring-ember-100 dark:bg-ink-900 dark:text-ink-50 dark:placeholder:text-ink-400">
        ...
        @error('password')
            <p class="mt-1 text-xs font-semibold text-ember-600 dark:text-ember-400" role="alert">{{ $message }}</p>
        @enderror
...
<p class="mt-6 text-center text-sm">
    <a href="{{ route('password.request') }}" class="font-bold text-ember-600 transition hover:text-ember-700 dark:text-ember-400 dark:hover:text-ember-300">{{ __('auth.forgot_password') }}</a>
</p>
@endsection
