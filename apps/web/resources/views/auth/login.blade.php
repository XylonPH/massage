...
@section('content')
<h1 class="text-2xl font-black tracking-tight text-ink-950">{{ __('auth.log_in_title') }}</h1>
<p class="mt-1.5 text-sm text-ink-500">{{ __('auth.log_in_subtitle') }}</p>

@if (session('verify_notice'))
    <p class="mt-5 rounded-xl border border-ink-200 bg-ink-50 px-4 py-3 text-sm font-semibold text-ink-700" role="status">
        {{ session('verify_notice') }}
    </p>
@endif

@if (session('password_reset_notice'))
    <p class="mt-5 rounded-xl border border-leaf-200 bg-leaf-50 px-4 py-3 text-sm font-semibold text-leaf-800" role="status">
        {{ session('password_reset_notice') }}
    </p>
@endif

@error('identifier')
    <p class="mt-5 rounded-xl border border-ember-200 bg-ember-50 px-4 py-3 text-sm font-semibold text-ember-800" role="alert">
        {{ $message }}
    </p>
@enderror
...
<form method="post" action="{{ route('login.store') }}" class="mt-7 space-y-5">
    ...
    <div>
        <label for="identifier" class="mb-1.5 block text-sm font-bold text-ink-900">{{ __('auth.identifier_label') }}</label>
        <input id="identifier" name="identifier" type="text" required autofocus autocomplete="username"
               value="{{ old('identifier') }}"
               class="w-full rounded-xl border border-ink-200 bg-white px-4 py-3 text-sm text-ink-950 placeholder-ink-400 transition focus:border-ember-400 focus:outline-none focus:ring-2 focus:ring-ember-100">
    </div>

    <div>
        <div class="mb-1.5 flex items-center justify-between">
            <label for="password" class="block text-sm font-bold text-ink-900">{{ __('auth.password_label') }}</label>
            <a href="{{ route('password.request') }}" class="text-xs font-semibold text-ember-600 transition hover:text-ember-700">{{ __('auth.forgot_password') }}</a>
        </div>
        <div class="relative">
...
<div class="mt-6 space-y-3 text-center text-sm">
    <p class="text-ink-500">
        {{ __('auth.no_account') }}
        <a href="{{ route('register') }}" class="font-bold text-ember-600 transition hover:text-ember-700">{{ __('auth.sign_up_link') }}</a>
    </p>
    <p><a href="{{ route('verification.notice') }}" class="text-xs font-semibold text-ink-400 transition hover:text-ink-600">{{ __('auth.resend_verification') }}</a></p>
</div>
@endsection
