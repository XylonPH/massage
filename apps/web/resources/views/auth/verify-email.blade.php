...
<div class="flex justify-center">
    <span class="flex size-14 items-center justify-center rounded-2xl bg-ember-50 text-ember-600">
...
<h1 class="mt-5 text-center text-2xl font-black tracking-tight text-ink-950">
...
@if (session('verify_notice'))
    <p class="mt-5 rounded-xl border border-ink-200 bg-ink-50 px-4 py-3 text-center text-sm font-semibold text-ink-700" role="status">
...
<form method="post" action="{{ route('verification.resend') }}" class="mt-7 space-y-4">
    @csrf
    <div>
        <label for="resend-email" class="mb-1.5 block text-sm font-bold text-ink-900">{{ __('auth.email_label') }}</label>
    ...
    <button type="submit" class="w-full rounded-xl border border-ink-200 px-4 py-3 text-sm font-bold text-ink-800 transition hover:border-ember-300 hover:bg-ember-50">
