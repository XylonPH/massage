...
<div class="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:px-8">
    <section aria-label="{{ $heading }}">
        @if ($articles->isEmpty())
            <div class="rounded-2xl border border-ink-100 bg-white p-10 text-center shadow-sm dark:border-ink-800 dark:bg-ink-900">
                <p class="text-lg font-semibold text-ink-700 dark:text-ink-200">{{ __('article.no_articles') }}</p>
            </div>
        @else
                ...
                @foreach ($articles as $item)
                    <article class="flex h-full flex-col overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-ink-800 dark:bg-ink-900">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            <h2 class="mt-4 text-2xl font-black leading-tight text-ink-950 dark:text-ink-50">
                                <a href="{{ route('article.show', $item['slug']) }}" class="hover:text-ember-600 dark:hover:text-ember-400">{{ $item['title'] }}</a>
                            </h2>
                            <p class="mt-3 flex-1 leading-7 text-charcoal-800 dark:text-ink-200">{{ $item['description'] }}</p>
                            <div class="mt-6 flex items-center justify-between gap-4 border-t border-ink-100 pt-4 text-sm text-ink-500 dark:border-ink-800 dark:text-ink-400">
    ...
    <aside class="space-y-6" aria-label="{{ __('article.browse_categories') }}">
        ...
        @if ($popularTags->isNotEmpty())
            <div class="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm dark:border-ink-800 dark:bg-ink-900">
                <h2 class="font-black text-ink-950 dark:text-ink-50">{{ __('article.browse_tags') }}</h2>
                <div class="mt-4 flex flex-wrap gap-2">
                    @foreach ($popularTags as $tag)
                        <a class="rounded-full border border-ink-200 px-3 py-1.5 text-sm text-ink-700 hover:border-ember-300 hover:text-ember-700 dark:border-ink-700 dark:text-ink-200 dark:hover:border-ember-700 dark:hover:text-ember-300" href="{{ route('article.tag.show', $tag->localized('tag_slug')) }}">#{{ $tag->localized('tag_title') }}</a>
                    @endforeach
                </div>
            </div>
        @endif
