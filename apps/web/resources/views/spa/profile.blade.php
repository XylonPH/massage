    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_21rem]">
        <div class="min-w-0 space-y-9">
            ...
            <section id="overview" role="tabpanel" aria-labelledby="spa-tab-overview" tabindex="0" data-spa-tab-panel="overview">
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm dark:border-ink-800 dark:bg-ink-900">
                    ...
                    <dl class="mt-6 grid grid-cols-2 gap-4 border-t border-ink-100 pt-5 sm:grid-cols-3 dark:border-ink-800">
                        @foreach ($spa['amenities'] as $amenity)
                            <div class="flex items-center gap-3">
                                <span class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-ink-50 dark:bg-ink-800">
                                    <x-pictogram :name="$amenity['icon']" class="size-5 text-ink-600 dark:text-ink-300" />
                                </span>
                                <div>
                                    <dt class="text-sm font-bold text-ink-900 dark:text-ink-100">{{ $amenity['label'] }}</dt>
                                    <dd class="text-xs text-leaf-600 dark:text-leaf-400">{{ $amenity['icon'] === 'card' ? __('spa.amenity_accepted') : __('spa.amenity_available') }}</dd>
                                </div>
                            </div>
                        @endforeach
                    </dl>
                    ...
                    @foreach ($spa['services'] as $service)
                        <li class="flex items-center gap-4 px-4 py-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-sm text-ink-400 dark:text-ink-300">{{ __('spa.minutes', ['count' => $service['duration']]) }}</p>
                            </div>
                            <p class="shrink-0 text-lg font-black text-ember-600 dark:text-ember-400">{{ $service['price'] }}</p>
                        </li>
                    @endforeach
                </ul>
                <p class="mt-3 text-xs text-ink-400 dark:text-ink-300">{{ __('spa.last_confirmed', ['date' => $spa['last_confirmed']]) }}</p>
            </section>
            ...
            <section id="therapists" role="tabpanel" aria-labelledby="spa-tab-therapists" tabindex="0" data-spa-tab-panel="therapists">
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    @foreach ($spa['therapists'] as $therapist)
                        <article class="rounded-2xl border border-ink-100 bg-white p-4 text-center shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-ink-800 dark:bg-ink-900">
                            ...
                            <h3 class="mt-3 font-bold text-ink-950 dark:text-ink-50">{{ $therapist['name'] }}</h3>
                            <p class="mt-0.5 text-xs font-bold uppercase tracking-wide text-ember-600 dark:text-ember-400">{{ $therapist['title'] }}</p>
                            <p class="mt-1.5 text-xs text-ink-500 dark:text-ink-400">{{ $therapist['specialties'] }}</p>
                            <div class="mt-2 flex items-center justify-center gap-1 text-xs">
                                <span class="font-bold text-ink-900 dark:text-ink-100">{{ number_format($therapist['rating'], 1) }}</span>
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    @foreach ($spa['treatment_areas'] as $area)
                        <article class="rounded-2xl border border-ink-100 bg-white p-4 shadow-sm dark:border-ink-800 dark:bg-ink-900">
                            <h3 class="font-bold text-ink-950 dark:text-ink-50">{{ $area['name'] }}</h3>
                            ...
                            <p class="mt-2.5 text-sm text-ink-500 dark:text-ink-400">{{ $area['note'] }}</p>
                        </article>
                    @endforeach
                </div>
            </section>
                ...
                <div class="rounded-2xl border border-ink-100 bg-white p-6 shadow-sm dark:border-ink-800 dark:bg-ink-900">
                    <div class="flex flex-col gap-8 md:flex-row">
                        <div class="shrink-0 text-center md:w-44">
                            <p class="text-sm font-bold text-ink-500 dark:text-ink-400">{{ __('spa.what_people_love') }}</p>
                            <p class="mt-3 rounded-xl bg-ink-50 px-3 py-4 text-sm font-semibold text-ink-500 dark:bg-ink-800 dark:text-ink-400">{{ __('spa.not_enough_ratings') }}</p>
                            <p class="mt-1.5 text-xs text-ink-400 dark:text-ink-300">{{ __('spa.rating_count', ['count' => $spa['rating_count']]) }}</p>
                            <p class="mt-1.5 text-xs text-ink-400 dark:text-ink-300">{{ __('common.reviews', ['count' => $spa['review_count']]) }}</p>
                        </div>
                        <ul class="min-w-0 flex-1 space-y-5">
                            ...
                            @empty
                                <li class="rounded-xl bg-ink-50 p-5 text-sm text-ink-500 dark:bg-ink-800 dark:text-ink-400">{{ __('spa.no_reviews') }}</li>
                            @endforelse
                        </ul>
                    </div>
                    <div class="mt-6 flex flex-wrap justify-end gap-3">
                        ...
                        <a href="{{ route('review.spa', ['target' => $spa['slug']]) }}" class="inline-flex items-center gap-1 rounded-xl border border-ink-200 px-4 py-2 text-sm font-bold text-ink-700 transition hover:border-ember-300 hover:text-ember-600 dark:border-ink-700 dark:text-ink-200 dark:hover:border-ember-700 dark:hover:text-ember-400">
            ...
            <section id="location" role="tabpanel" aria-labelledby="spa-tab-location" tabindex="0" data-spa-tab-panel="location">
                ...
                <div class="overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm dark:border-ink-800 dark:bg-ink-900">
                    ...
                    <p class="border-b border-ink-100 px-4 py-2.5 text-xs text-ink-500 dark:border-ink-800 dark:text-ink-400">{{ __('spa.map_location_note') }}</p>
                    <div class="grid gap-5 p-5 sm:grid-cols-2">
                        <div class="space-y-5">
                            <div>
                                <h3 class="text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.public_address') }}</h3>
                                <p class="mt-2 text-sm leading-relaxed text-ink-700 dark:text-ink-200">{{ $spa['address_public'] }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.directions_note') }}</h3>
                                <p class="mt-2 text-sm leading-relaxed text-ink-600 dark:text-ink-300">{{ $spa['direction_note'] }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.parking_information') }}</h3>
                                <p class="mt-2 text-sm leading-relaxed text-ink-600 dark:text-ink-300">{{ $spa['parking_note'] }}</p>
                            </div>
                            <a href="{{ $spa['map_url'] }}" target="_blank" rel="nofollow noopener noreferrer" class="inline-flex items-center gap-1.5 text-sm font-bold text-ember-600 transition hover:text-ember-700 dark:text-ember-400 dark:hover:text-ember-300">
                                {{ __('common.get_directions') }}
                                <svg viewBox="0 0 20 20" fill="currentColor" class="size-4" aria-hidden="true"><path fill-rule="evenodd" d="M3 10a.75.75 0 0 1 .75-.75h9.69L10.22 6.03a.75.75 0 1 1 1.06-1.06l4.5 4.5a.75.75 0 0 1 0 1.06l-4.5 4.5a.75.75 0 1 1-1.06-1.06l3.22-3.22H3.75A.75.75 0 0 1 3 10Z" clip-rule="evenodd"/></svg>
                            </a>
                        </div>
                        <div>
                            <h3 class="text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.nearby_landmarks') }}</h3>
                            <ul class="mt-2 space-y-1.5 text-sm text-ink-600 dark:text-ink-300">
                    ...
                    <div id="contact" class="scroll-mt-32 border-t border-ink-100 p-5 dark:border-ink-800">
                        <h3 class="text-base font-black text-ink-950 dark:text-ink-50">{{ __('spa.contact_title') }}</h3>
                        <p class="mt-1 text-xs text-ink-500 dark:text-ink-400">{{ __('spa.contact_channel_notice') }}</p>
                            ...
                            @foreach ($spa['contact_channel_list'] as $channel)
                                <li>
                                    <a href="{{ $channel['contact_url'] }}"
                                       ...
                                       class="flex h-full items-center justify-between gap-3 rounded-xl border border-ink-100 px-4 py-3 transition hover:border-ember-200 hover:bg-ember-50/40 dark:border-ink-800 dark:hover:border-ember-800 dark:hover:bg-ember-950/40">
                                        <span class="min-w-0">
                                            <span class="block text-xs font-bold uppercase tracking-wide text-ink-500 dark:text-ink-400">{{ $channel['contact_label'] }}</span>
                                            <span class="mt-0.5 block truncate text-sm font-semibold text-ink-900 dark:text-ink-100">{{ $channel['contact_value'] }}</span>
                                        </span>
            ...
            <section aria-labelledby="verification-heading">
                ...
                <div class="flex flex-wrap items-center gap-4 rounded-2xl border border-ink-100 bg-white p-5 text-sm text-ink-600 shadow-sm dark:border-ink-800 dark:bg-ink-900 dark:text-ink-300">
        ...
        <aside class="min-w-0">
            <div class="space-y-6 lg:sticky lg:top-36">
                <section id="book" aria-labelledby="book-heading" class="scroll-mt-32 rounded-2xl border border-ink-100 bg-white p-4 shadow-md dark:border-ink-800 dark:bg-ink-900">
                    <h2 id="book-heading" class="text-lg font-black text-ink-950 dark:text-ink-50">{{ __('spa.book_title') }}</h2>
                    <form class="mt-5 space-y-4" action="#book" method="get">
                        <div>
                            <label for="booking-service" class="mb-1 block text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.select_service') }}</label>
                        ...
                        <div>
                            <label for="booking-date" class="mb-1 block text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.select_date') }}</label>
                        ...
                        <div>
                            <label for="booking-time" class="mb-1 block text-sm font-bold text-ink-900 dark:text-ink-100">{{ __('spa.select_time') }}</label>
                        ...
                        <p class="text-center text-xs text-ink-400 dark:text-ink-300">{{ __('spa.book_note') }}</p>
                        <p class="rounded-xl bg-ink-50 px-3.5 py-2.5 text-center text-xs text-ink-500 dark:bg-ink-800 dark:text-ink-400">{{ __('spa.booking_mode_request') }}</p>
                    </form>
                </section>
