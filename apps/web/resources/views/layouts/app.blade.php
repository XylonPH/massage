...
<body class="min-h-screen bg-slate-50 font-sans text-charcoal-900 antialiased dark:bg-charcoal-950 dark:text-ink-50">
    ...
    {{-- ===================== Header ===================== --}}
    <header class="sticky top-0 z-40 border-b border-ink-100 bg-white/95 shadow-sm backdrop-blur dark:border-ink-800 dark:bg-ink-900/95">
        <div class="mx-auto flex h-[4.5rem] max-w-[1600px] items-center gap-5 px-4 sm:px-6 lg:px-8">
            ...
            <nav class="hidden flex-1 items-center gap-1 xl:flex" aria-label="{{ __('navigation.main_navigation') }}">
                @foreach ($mainNavigation as $item)
                    <a href="{{ $item['href'] }}"
                       ...
                       class="rounded-lg px-3 py-2 text-sm font-semibold transition {{ $item['active'] ? 'text-ember-600 dark:text-ember-400' : 'text-ink-800 hover:bg-ink-50 hover:text-ink-950 dark:text-ink-200 dark:hover:bg-ink-800 dark:hover:text-ink-50' }}">
                        {{ $item['label'] }}
                    </a>
                @endforeach
            </nav>
            ...
            <div class="ml-auto hidden items-center gap-2.5 xl:flex">
                ...
                @else
                    <a href="{{ route('login') }}" class="rounded-lg border border-ink-200 px-4 py-2 text-sm font-semibold text-ink-800 transition hover:border-ink-300 hover:bg-ink-50 dark:border-ink-700 dark:text-ink-200 dark:hover:border-ink-600 dark:hover:bg-ink-800">
            ...
            <button type="button" data-menu-toggle aria-expanded="false" aria-controls="mobile-menu"
                    class="ml-auto inline-flex items-center justify-center rounded-lg p-2 text-ink-800 hover:bg-ink-50 xl:hidden dark:text-ink-200 dark:hover:bg-ink-800">
        ...
        <div id="mobile-menu" hidden class="border-t border-ink-100 bg-white xl:hidden dark:border-ink-800 dark:bg-ink-900">
            <nav class="space-y-1 px-4 py-3" aria-label="{{ __('navigation.main_navigation') }}">
                <a href="{{ route('home') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-ink-900 hover:bg-ink-50 dark:text-ink-100 dark:hover:bg-ink-800">{{ __('navigation.home') }}</a>
                <a href="{{ route('directory.index') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-ink-900 hover:bg-ink-50 dark:text-ink-100 dark:hover:bg-ink-800">{{ __('navigation.directory') }}</a>
                <a href="{{ route('article.index') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-ink-900 hover:bg-ink-50 dark:text-ink-100 dark:hover:bg-ink-800">{{ __('navigation.articles') }}</a>
                <a href="{{ route('campus.index') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-ink-900 hover:bg-ink-50 dark:text-ink-100 dark:hover:bg-ink-800">{{ __('navigation.campus') }}</a>
                <a href="{{ route('promo.index') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-ink-900 hover:bg-ink-50 dark:text-ink-100 dark:hover:bg-ink-800">{{ __('navigation.promos') }}</a>
                <div class="pt-2"><x-theme-toggle class="size-11" /></div>
                @auth
                    @if (auth()->user()->isActive() && auth()->user()->hasVerifiedEmail())
                        <a href="{{ route('workspace.home') }}" class="block rounded-lg px-3 py-2 text-sm font-semibold text-ink-900 hover:bg-ink-50 dark:text-ink-100 dark:hover:bg-ink-800">{{ __('navigation.workspace') }}</a>
                    @endif
                    <div class="flex items-center gap-2.5 pt-2">
                        <x-identity-capsule :user="auth()->user()" class="min-w-0 flex-1" />
                        <form method="post" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="inline-flex size-11 items-center justify-center rounded-full border border-ink-200 text-ink-700 dark:border-ink-700 dark:text-ink-200" aria-label="{{ __('auth.log_out') }}">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="size-5" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M14 8V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h7a2 2 0 0 0 2-2v-3M10 12h11m0 0-3-3m3 3-3 3"/></svg>
                            </button>
                        </form>
                    </div>
                @else
                    <div class="flex gap-2.5 pt-2">
                        <a href="{{ route('login') }}" class="flex-1 rounded-lg border border-ink-200 px-4 py-2 text-center text-sm font-semibold text-ink-800 dark:border-ink-700 dark:text-ink-200">{{ __('navigation.log_in') }}</a>
