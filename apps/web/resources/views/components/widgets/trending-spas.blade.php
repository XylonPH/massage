<section class="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
    ...
    <ul class="mt-5 space-y-4">
        @foreach ([
            ...
            <li>
                <a href="#" class="group flex items-start gap-3 rounded-xl p-2 transition-all duration-300 ease-out hover:bg-ink-50 hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]">
                    <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-ink-100 text-xs font-bold text-ink-500 transition-colors duration-300 group-hover:bg-ember-100 group-hover:text-ember-700">{{ $index + 1 }}</span>
                    <div class="min-w-0 flex-1">
                        <p class="truncate text-sm font-bold text-ink-900 group-hover:text-ember-700">{{ $spa['name'] }}</p>
                        <p class="mt-0.5 flex items-center gap-1 text-xs text-ink-500">
                            <span class="font-bold text-ink-700">{{ $spa['rating'] }}</span>
