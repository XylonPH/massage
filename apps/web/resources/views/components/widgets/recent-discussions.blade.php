<section class="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm dark:border-ink-800 dark:bg-ink-900">
    <div class="flex items-center justify-between">
        <h3 class="text-base font-black tracking-tight text-ink-950 dark:text-ink-50">{{ __('widget.recent_discussions') }}</h3>
    </div>

    <ul class="mt-5 divide-y divide-ink-50 dark:divide-ink-800">
        @foreach ([
            ...
            <li class="py-3 first:pt-0 last:pb-0">
                <a href="#" class="group block transition">
                    <h4 class="text-sm font-bold leading-snug text-ink-900 group-hover:text-ember-700 dark:text-ink-100 dark:group-hover:text-ember-400">{{ $discussion['title'] }}</h4>
                    <p class="mt-1 flex items-center gap-2 text-xs text-ink-500 dark:text-ink-400">
                        <span class="flex items-center gap-1 font-semibold text-ink-700 dark:text-ink-200">
