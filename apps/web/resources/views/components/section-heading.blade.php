...
<div {{ $attributes->merge(['class' => 'mb-3 flex items-end justify-between gap-4']) }}>
    <h2 class="flex items-center gap-2.5 text-lg font-bold tracking-tight text-ink-950">
        <span class="inline-block h-5 w-1.5 rounded-full {{ $accent === 'leaf' ? 'bg-leaf-500' : 'bg-ember-500' }}" aria-hidden="true"></span>
        {{ $title }}
    </h2>
    @if ($href)
        <a href="{{ $href }}" class="group inline-flex shrink-0 items-center gap-1 text-sm font-semibold text-ink-600 transition hover:text-ember-600">
            {{ __('common.view_all') }}
