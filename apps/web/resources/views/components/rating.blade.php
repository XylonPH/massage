...
<span {{ $attributes->merge(['class' => 'inline-flex items-center gap-0.5']) }} role="img" aria-label="{{ __('common.rating_out_of_five', ['rating' => number_format($value, 1)]) }}">
    @for ($i = 1; $i <= 5; $i++)
        ...
        @else
            <svg viewBox="0 0 20 20" fill="currentColor" class="{{ $size }} text-ink-200" aria-hidden="true"><path d="M9.05 2.93c.3-.92 1.6-.92 1.9 0l1.28 3.95a1 1 0 0 0 .95.69h4.16c.97 0 1.37 1.24.59 1.81l-3.37 2.44a1 1 0 0 0-.36 1.12l1.28 3.95c.3.92-.75 1.69-1.54 1.12l-3.36-2.44a1 1 0 0 0-1.18 0l-3.36 2.44c-.79.57-1.84-.2-1.54-1.12l1.28-3.95a1 1 0 0 0-.36-1.12L2.05 9.38c-.78-.57-.38-1.81.6-1.81h4.15a1 1 0 0 0 .95-.69l1.3-3.95Z"/></svg>
        @endif
    @endfor
</span>
