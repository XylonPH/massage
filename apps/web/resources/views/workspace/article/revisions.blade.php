...
<div class="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
    <div class="grid gap-6 lg:grid-cols-[16rem_minmax(0,1fr)]">
        ...
        <main class="min-w-0">
            <a href="{{ route('workspace.article.edit', $article) }}" class="text-sm font-bold text-ember-700 hover:underline">&larr; {{ __('article.back_to_editor') }}</a>
            <h1 class="mt-3 text-3xl font-black text-ink-950">{{ __('article.revision_history') }}</h1>
            <p class="mt-1 text-lg text-ink-600">{{ $article->localized('article_title') }}</p>
            <ol class="mt-6 space-y-4">
                @foreach ($revisions as $revision)
                    <li class="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                            <h2 class="text-lg font-black text-ink-950">{{ __('article.revision_number', ['number' => $revision->revision_number]) }}</h2>
