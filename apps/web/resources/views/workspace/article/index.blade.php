...
<div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
            <p class="text-sm font-bold uppercase tracking-[0.18em] text-ember-600">{{ __('navigation.articles') }}</p>
            <h1 class="mt-2 text-4xl font-black text-ink-950">{{ __('article.workspace_title') }}</h1>
            <p class="mt-2 text-ink-600">{{ __('article.workspace_intro') }}</p>
        </div>
        <a href="{{ route('workspace.article.create') }}" class="inline-flex items-center justify-center rounded-xl bg-ember-500 px-5 py-3 font-bold text-white shadow-sm hover:bg-ember-600">{{ __('article.new_article') }}</a>
    </div>

    <nav class="mt-8 flex flex-wrap gap-2 border-b border-ink-100 pb-3" aria-label="{{ __('article.status') }}">
        @foreach ([[null, __('article.all')], ['draft', __('article.drafts')], ['submitted', __('article.submitted')], ['published', __('article.published_tab')]] as [$key, $label])
            ...
            <a href="{{ route($route) }}" @if ($status === $key) aria-current="page" @endif
               class="rounded-lg px-4 py-2 text-sm font-bold {{ $status === $key ? 'bg-ink-900 text-white' : 'bg-ink-50 text-ink-700 hover:bg-ink-100' }}">{{ $label }}</a>
        @endforeach
    </nav>

    <div class="mt-6 overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm">
        @if ($articles->isEmpty())
            <p class="p-10 text-center font-semibold text-ink-600">{{ __('article.no_workspace_articles') }}</p>
        @else
                    ...
                    <tbody class="divide-y divide-ink-100">
                        @foreach ($articles as $item)
                            <tr>
                                ...
                                <td class="px-5 py-4 text-sm text-ink-600">{{ $status === 'submitted' ? __('article.submitted') : (['D' => __('article.drafts'), 'S' => 'Scheduled', 'P' => __('article.published_tab'), 'U' => 'Unpublished'][$item->status_publication] ?? $item->status_publication) }}</td>
