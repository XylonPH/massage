 ...
 * - MFA means multi-factor authentication: after the password, the user must provide a second proof.
 * - One record represents one enrolled second-factor method, such as a TOTP authenticator app.
 * - TOTP means a short time-based one-time code generated by an authenticator app.
 * - A pending method must not satisfy an MFA challenge until verified_at is recorded and the status is Active.
 * - secret_reference is only an opaque pointer to separately encrypted secret material. The reusable secret must not be stored here in readable form.
 * - Recovery codes are single-use backup proofs stored as hashes in user_recovery_code; they are not MFA methods and are not password-reset tokens.
 * - Password-reset tokens are delivered through the separate email recovery workflow and do not belong in this collection.
 * - MFA enrollment and challenge enforcement are not enabled merely because this structure guide exists; runtime verification and recovery safeguards are required first.
 */
$created_at = '2026-07-22T02:51:15Z';
$updated_at = '2026-07-22T06:30:55Z';
$user_mfa_method_default = [
    'is_primary' => false,
    'status_user_mfa_method' => 'PND',
    'revision_number' => 1,
];

$user_mfa_method = [
    '_id' => 'Mf7K2pQ9xR4tV8zN',
    'user_id' => 'U2pR7vX4kT9mC5qL',
    'type_mfa_method' => 'TOTP',
    'method_label' => 'Authenticator app',
    'secret_reference' => 'vault://mfa/example', // Example opaque reference only; never store the readable TOTP secret here.
    'is_primary' => true,
    'status_user_mfa_method' => 'ACT',
    'verified_at' => '2026-07-22T02:51:15Z',
    'last_used_at' => null,
    'disabled_at' => null,
    'revision_number' => 1,
    'created_at' => '2026-07-22T02:51:15Z',
...
$user_mfa_method_field_order = [
    '_id', 'user_id', 'type_mfa_method', 'method_label', 'secret_reference',
    'is_primary', 'status_user_mfa_method', 'verified_at', 'last_used_at',
    'disabled_at', 'revision_number', 'created_at', 'updated_at',
];
